...
export async function getArticleBySlug(
  ...
  const query = `*[_type == "article" && !(_id in path('drafts.**')) && slug.current == $slug ${isActiveQuery}]{_createdAt,_id,intro,metaTitle,metaDesc,title,isActive,content,slug,imageMain{asset->{path,url}}, articleCategory->{title,slug,_ref, _id} }`;
